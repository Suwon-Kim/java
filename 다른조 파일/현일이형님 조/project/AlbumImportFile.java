...
public class AlbumImportFile extends JDialog {
	...
	public AlbumImportFile(AlbumMain albumMain) {
		super(albumMain, "사진첩 - 파일 추가", true);
...
//		btnFileImport.setMargin(new Insets(1,5,1,5));
//		btnFileImport.setPreferredSize(new Dimension(40,30));
...
//		pnlCheckBox.setPreferredSize(new Dimension(1,1));
	...
	private void addListener() {
		...
		btnFileImport.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				JFileChooser fileChooser = new JFileChooser(".//pic//");
				fileChooser.setMultiSelectionEnabled(true);
				int result = fileChooser.showOpenDialog(AlbumImportFile.this);
				if (result == JFileChooser.APPROVE_OPTION) {
					importFiles = fileChooser.getSelectedFiles();
					int fileCount = 0;
					for (File file : importFiles) {
						for (String ext : AlbumUtil.IMG_EXT_ABLE) {
							if (file.getName().toUpperCase().contains(ext.toUpperCase())) {
								flag = true;
						...
						if (fileCount > 1) {
							filePath = "파일이 여러개 선택 되었습니다";
						}
					}
					if (flag) {
						tfFilePath.setText(filePath);
						tfHashtag.setEditable(true);
					} else {
						JOptionPane.showMessageDialog(albumMain, AlbumUtil.EXT_WARNNING + "파일만 지원합니다", "경고",
		...
		btnApply.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				ArrayList<File> importFileArrayList;
				importFileArrayList = importFileSameDeleteList();
				...
				for(String checkedGroupName : cbCheckedGroupList){
					for(AlbumGroup groupInfo : albumGroupList){
						if(groupInfo.getName().equals(checkedGroupName)){
							for(File fileName : importFileArrayList){
								groupInfo.getPictureInGroup().add(fileName.getPath());
							}
						}
					}
				}
	...
	private ActionListener alGroupAdd(){
		return new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				new AlbumGroupCreateDialog(albumMain,AlbumImportFile.this);
			}
		};
	}
	...
	private ArrayList<File> importFileSameDeleteList() {
		...
		if(sameFileCount!=0){
			JOptionPane.showMessageDialog(AlbumImportFile.this, "중복파일 " + sameFileCount + "는 제거 되었습니다", "알림", JOptionPane.YES_OPTION);
		}
