...
public class WaitingRoom_GUI extends JFrame {
	...
	public WaitingRoom_GUI(Socket sock, ObjectOutputStream oos, ObjectInputStream ois, Vector<User> waitUserList,
		...
		System.out.println("waiting gui -> waitUserList" + waitUserList);
		System.out.println("waiting gui -> allChatRoomList" + allChatRoomList);
		this.sock = sock;
		this.oos = oos;
		this.ois = ois;
		this.name = name;
		...
		ActionListener alistener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (obj == btnCreateRoom) {
					// 방 생성
					new CreateRoom_GUI(sock, oos, ois, name);
					// setVisible(false);
				} else if (obj == btnExit) {
...
//							throw new Exception();
					...
					send = new SendData(210, chatTitle);
					try {
						oos.writeObject(send);
						oos.flush();
						oos.reset();
			...
			public void keyPressed(KeyEvent e) {
				...
				if (key == KeyEvent.VK_ENTER) {
					if (cbInputOption.getSelectedItem().equals("귓속말")) {
						...
						SendData send = new SendData(202, msg);
						try {
							oos.writeObject(send);
							oos.flush();
							oos.reset();
						...
						if (msg.trim().length() > 0) {
							SendData send = new SendData(201, msg);
							try {
								oos.writeObject(send);
								oos.flush();
								oos.reset();
	...
	class WinInputThread extends Thread {
		...
		public void run() {
			...
			// String threadName = Thread.currentThread().getName();//스레드 풀에 있는
			// 해당 스레드 이름 얻기
...
//						taWaitingRoomChat.setForeground(Color.BLUE);
					...
					case 1010:
						...
						System.out.println("방에있는 유저리스트" + ri.getChatRoomUserList());
					...
					case 1070:
						...
						int result = JOptionPane.showConfirmDialog(WaitingRoom_GUI.this,
								RoomTitle + "번방" + ", " + SendUser + "님의 초대를 수락하시겠습니까?", "초대", JOptionPane.YES_NO_OPTION,
								JOptionPane.PLAIN_MESSAGE);
 
						if (result == 0) {
							// 수락
							SendData send = new SendData(171, RoomTitle, name, SendUser);
							try {
								oos.writeObject(send);
								oos.flush();
								oos.reset();
							...
							// 거절
							SendData send = new SendData(172, SendUser);
							try {
								oos.writeObject(send);
								oos.flush();
								oos.reset();
...
//
//							/*
//							 * for(ChatRoomInfo c:vec) { for (int i = 0; i < allChatRoomList.size(); i++) {
//							 * if(allChatRoomList.get(i).getTitle().equals(c.getTitle())) { Vector<String>
//							 * info = new Vector<String>(); info.add(allChatRoomList.get(i).getTitle());
//							 * info.add(allChatRoomList.get(i).getChatRoomUserList().size() + "/" +
//							 * allChatRoomList.get(i).getMaxPeople());
//							 * info.add(allChatRoomList.get(i).getChatRoom_host()); model.addRow(colName); }
//							 * } }
//							 */
//
//							for (int i = 0; i < vec.size(); i++) {
						...
						if(count == 0) {
								 ...
								 for(int j=0;j<vec.size(); j++) {
									for(int i =0;i<allChatRoomList.size();i++) {
											...
											info.add(allChatRoomList.get(i).getChatRoomUserList().size() + "/" + allChatRoomList.get(i).getMaxPeople());
